...
function Product_listing({
  ...
  useEffect(() => {
    ...
    dispatch({ type: "SET_FILTERED_PRODUCTS", payload: storedProducts });
  }, []);
  ...
  const handleCategoryClick = (categoryId) => {
    if (selectedCategories.includes(categoryId)) {
      dispatch({
        ...
        payload: selectedCategories.filter((cat) => cat !== categoryId),
      });
    } else {
      dispatch({
        type: "SET_SELECTED_CATEGORIES",
        payload: [...selectedCategories, categoryId],
      });
    }
  };
  ...
  useEffect(() => {
    const filtered = products.filter((product) => {
      const categoryFilter =
        ...
        selectedCategories.some((cat) => cat === product.category_id);

      const priceRangeFilter =
        product.new_price >= priceFilter[0] &&
        product.new_price <= priceFilter[1];
  ...
  }, [currentPage, selectedCategories, priceFilter, products]);
  ...
  const [activeAccordions, setActiveAccordions] = useState([1, 2, 3, 4]);
